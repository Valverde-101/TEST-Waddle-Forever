...
# Music is intentionally generated from one authoritative ID list. Validate the
# full set plus the generator contract instead of requiring the old expanded
# `P + ...` literals, which made this gate lie after ref() was introduced.
...
$expectedList = 'const HALLOWEEN_2015_MUSIC_IDS = [' + (($musicIds | ForEach-Object {[string]$_}) -join ', ') + '];'
Require ($updates.Contains($expectedList)) 'music_id_set_38'
Require ($updates.Contains('const musicFileChanges = Object.fromEntries(')) 'music_generator'
Require ($updates.Contains('HALLOWEEN_2015_MUSIC_IDS.map(id => [')) 'music_generator_ids'
...
Require ($updates.Contains("'play/v2/content/global/membership/party1.swf': ref('membership/MembershipParty1-HalloweenParty2015.swf')")) 'membership_party1'
Require ($updates.Contains("'play/v2/content/global/membership/party2.swf': ref('membership/MembershipParty2-HalloweenParty2015.swf')")) 'membership_party2'
