...
function Test-WaddleManagedNodeContract {
  $repo = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))
  $configPath = Join-Path $repo '.androidbuild.json'
  if (-not (Test-Path -LiteralPath $configPath -PathType Leaf)) { throw "WADDLE_MANAGED_NODE=FAIL config_missing=$configPath" }
  $config = Get-Content -LiteralPath $configPath -Raw | ConvertFrom-Json
  $declaredNode = [string]$config.toolchain.node
  $declaredYarn = [string]$config.toolchain.yarn
  if ($declaredNode -ne $script:WaddleNodeVersion) { throw "WADDLE_MANAGED_NODE=FAIL contract_node expected=$script:WaddleNodeVersion actual=$declaredNode" }
  if ($declaredYarn -ne $script:WaddleYarnVersion) { throw "WADDLE_MANAGED_YARN=FAIL contract_yarn expected=$script:WaddleYarnVersion actual=$declaredYarn" }
}
...
function Get-WaddleManagedNodeLayout {
  ...
  $nodeHome = [IO.Path]::GetFullPath((Join-Path $toolBase ($script:WaddleNodeVersion + '\x64')))
  [pscustomobject]@{
    tool_base = $toolBase
    home = $nodeHome
    ...
    manifest = (Join-Path $nodeHome 'androidbuild-waddle-node.json')
  }
}
...
function Install-WaddleManagedNodeFromCandidate {
  ...
  try {
    ...
    if (-not (Test-WaddleNodeExecutable -Path (Join-Path $stage 'node.exe'))) { throw "WADDLE_MANAGED_NODE=FAIL candidate_copy_invalid source=$CandidateNode" }
...
function Install-WaddleManagedNodeFromOfficialArchive {
  ...
  if (Test-Path -LiteralPath $archive -PathType Leaf) {
    ...
    if ($hash -eq $script:WaddleNodeArchiveSha256) { $downloadRequired = $false } else { Remove-Item -LiteralPath $archive -Force }
  }
  if ($downloadRequired) {
  ...
  if ($actualHash -ne $script:WaddleNodeArchiveSha256) { throw "WADDLE_MANAGED_NODE=FAIL archive_sha256 expected=$script:WaddleNodeArchiveSha256 actual=$actualHash path=$archive" }
  ...
  try {
    ...
    if (-not (Test-WaddleNodeExecutable -Path $node)) { throw "WADDLE_MANAGED_NODE=FAIL archive_extract_invalid path=$node" }
    if (Test-Path -LiteralPath $Layout.home) { Remove-Item -LiteralPath $Layout.home -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $Layout.home) | Out-Null
...
function Ensure-WaddleManagedYarn {
  param([Parameter(Mandatory)][pscustomobject]$Layout)
  if (-not (Test-WaddleNodeExecutable -Path $Layout.node)) { throw "WADDLE_MANAGED_YARN=FAIL node_invalid=$($Layout.node)" }
  $valid = $false
  if (Test-Path -LiteralPath $Layout.yarn -PathType Leaf) {
    try { $actual = (& $Layout.yarn --version 2>$null).Trim(); $valid = $actual -eq $script:WaddleYarnVersion } catch {}
  }
  if ($valid) { Write-Host "WADDLE_MANAGED_YARN=PASS version=$script:WaddleYarnVersion mode=existing path=$($Layout.yarn)"; return }
  if (-not (Test-Path -LiteralPath $Layout.npm -PathType Leaf)) { throw "WADDLE_MANAGED_YARN=FAIL npm_missing=$($Layout.npm)" }
  Write-Host "WADDLE_MANAGED_YARN_INSTALL=START version=$script:WaddleYarnVersion prefix=$($Layout.home)"
  $npmProcess = Start-Process -FilePath $Layout.npm -ArgumentList @('install','--global',("yarn@" + $script:WaddleYarnVersion),'--prefix',$Layout.home,'--no-audit','--no-fund') -Wait -PassThru -NoNewWindow
  if ($npmProcess.ExitCode -ne 0) { throw "WADDLE_MANAGED_YARN=FAIL npm_exit=$($npmProcess.ExitCode)" }
  if (-not (Test-Path -LiteralPath $Layout.yarn -PathType Leaf)) { throw "WADDLE_MANAGED_YARN=FAIL yarn_missing_after_install=$($Layout.yarn)" }
  $actual = (& $Layout.yarn --version).Trim()
  if ($actual -ne $script:WaddleYarnVersion) { throw "WADDLE_MANAGED_YARN=FAIL required=$script:WaddleYarnVersion actual=$actual path=$($Layout.yarn)" }
...
function Ensure-WaddleManagedNodeToolchain {
  param([Parameter(Mandatory)][string]$AndroidBuildRoot)
  Test-WaddleManagedNodeContract
  ...
  $mutex = New-Object -TypeName System.Threading.Mutex -ArgumentList $false,'AndroidBuild-Waddle-Node-20.19.0-x64'
  $acquired = $false
  try {
    ...
    if (-not (Test-WaddleNodeExecutable -Path $layout.node)) {
      ...
      if ($candidate) { $source = Install-WaddleManagedNodeFromCandidate -CandidateNode $candidate -Layout $layout }
      else { $source = Install-WaddleManagedNodeFromOfficialArchive -Layout $layout }
    }
    if (-not (Test-WaddleNodeExecutable -Path $layout.node)) { throw "WADDLE_MANAGED_NODE=FAIL final_validation path=$($layout.node)" }
  ...
  [pscustomobject]@{ status='PASS'; node_version=$script:WaddleNodeVersion; yarn_version=$script:WaddleYarnVersion; home=$layout.home; node=$layout.node; npm=$layout.npm; yarn=$layout.yarn; manifest=$layout.manifest }
}
...
function Enable-WaddleManagedNodeToolchain {
  ...
  if ($nodeVersion -ne $script:WaddleNodeVersion) { throw "WADDLE_MANAGED_NODE=FAIL selected_version required=$script:WaddleNodeVersion actual=$nodeVersion path=$selectedNode" }
  if ($yarnVersion -ne $script:WaddleYarnVersion) { throw "WADDLE_MANAGED_YARN=FAIL selected_version required=$script:WaddleYarnVersion actual=$yarnVersion path=$selectedYarn" }
  if ([IO.Path]::GetFullPath($selectedNode) -ne [IO.Path]::GetFullPath($managed.node)) { throw "WADDLE_MANAGED_NODE=FAIL selection_not_managed selected=$selectedNode expected=$($managed.node)" }
