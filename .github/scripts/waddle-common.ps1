...
function Get-WaddleContext {
  ...
  if (-not (Test-Path -LiteralPath $ContextPath -PathType Leaf)) { throw "WADDLE_CONTEXT=FAIL missing=$ContextPath" }
...
function Resolve-WaddleRepoRoot {
  ...
  if ($Context.ContainsKey('repo_root') -and $Context.repo_root) { return [IO.Path]::GetFullPath([string]$Context.repo_root) }
...
function Resolve-WaddleAndroidBuildRoot {
  ...
  if ($Context.ContainsKey('androidbuild_root') -and $Context.androidbuild_root) { $candidates.Add([string]$Context.androidbuild_root) }
  if ($env:ANDROIDBUILD_ROOT -and -not $candidates.Contains([string]$env:ANDROIDBUILD_ROOT)) { $candidates.Add([string]$env:ANDROIDBUILD_ROOT) }
  try {
  ...
  foreach ($candidate in $candidates) {
    ...
    if (Test-Path -LiteralPath $module -PathType Leaf) { return [IO.Path]::GetFullPath($full) }
  }
...
function Get-WaddleRequiredCoreVersion {
  ...
  if (-not (Test-Path -LiteralPath $configPath -PathType Leaf)) { throw "ANDROIDBUILD_CONFIG=FAIL missing=$configPath" }
  $config = Get-Content -LiteralPath $configPath -Raw | ConvertFrom-Json
  $required = [string]$config.core.minimum_version
  if ([string]::IsNullOrWhiteSpace($required)) { throw 'ANDROIDBUILD_CONFIG=FAIL core.minimum_version_missing' }
...
function Ensure-WaddleJunction {
  ...
  if (Test-Path -LiteralPath $LinkPath) {
    ...
    if (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
      ...
      $actual = if ([IO.Path]::IsPathRooted([string]$actualTargetRaw)) { [IO.Path]::GetFullPath([string]$actualTargetRaw) } else { [IO.Path]::GetFullPath((Join-Path $item.Parent.FullName ([string]$actualTargetRaw))) }
      ...
      if ($AllowPhysicalDirectory) {
        Write-Host "WORK_PATH=PASS path=$LinkPath target=$target mode=physical_existing"
        return
      }
  ...
  try {
    ...
    if (-not $AllowPhysicalDirectory) { throw }
    if (-not (Test-Path -LiteralPath $LinkPath)) { New-Item -ItemType Directory -Force -Path $LinkPath | Out-Null }
    Write-Host "WORK_PATH=PASS path=$LinkPath target=$target mode=physical_fallback reason=$($_.Exception.Message)"
  }
}
...
function Initialize-WaddleWorkspace {
  ...
  foreach ($relative in @(
    'dependencies\current\node_modules','cache\yarn','runtime\interactive','downloads','content','swf-analysis','diagnostics','logs\build','logs\runtime','dist\package','tmp'
  )) { Ensure-WaddleDirectory (Join-Path $work $relative) }

  $legacyDependencies = Join-Path $work 'dependencies\node_modules'
  if (Test-Path -LiteralPath $legacyDependencies) { Write-Host "WADDLE_LEGACY_DEPENDENCIES=INFO path=$legacyDependencies action=preserved_lock_safe_migration" }
  ...
  [pscustomobject]@{ status='PASS'; repo_root=$RepoRoot; work_root=$work; node_modules=$currentModules; yarn_cache=$env:YARN_CACHE_FOLDER }
}
...
function Test-WaddleToolchain {
  ...
  [pscustomobject]@{ status='PASS'; node=$nodeVersion; yarn=$yarnVersion; npm=$npmVersion; ffdec=$ffdec.path }
}
