...
foreach ($target in $targets) {
  ...
  if ([string]$target.role -match '^dialogue-(gary|aa|rh|cad|dot|sen|ph|rook)_congrats$') {
    if ($evidence.files -lt 1) { throw "WADDLE_PARTY2015_PROTOCOL=FAIL reward_dialogue_unscripted role=$($target.role)" }
...
# The Halloween namespace contains both localized text keys and content crumbs.
# Validate the complete 38-string contract from the versioned JSON, then classify
# static SWF evidence without pretending every crumb is a localized string.
...
if ($null -eq $localizationContract.strings) { throw 'WADDLE_PARTY2015_PROTOCOL=FAIL localization_contract_strings_missing' }
...
if ($observedLocalizationKeys.Count -lt 1) { throw 'WADDLE_PARTY2015_PROTOCOL=FAIL no_localization_evidence_in_preserved_swfs' }
...
$summary = [ordered]@{
  ...
  namespaceTokens=@($localizations | Sort-Object); localizationTokens=@($observedLocalizationKeys | Sort-Object);
  contentCrumbTokens=@($contentCrumbTokens | Sort-Object); localizationContractCount=$expectedLocalizationKeys.Count;
