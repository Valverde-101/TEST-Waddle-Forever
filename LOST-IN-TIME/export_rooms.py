...
def build_records(
    ...
    for idx, event in enumerate(all_events, start=1):
        ...
        if file_ref:
            ...
            swf_type = prefix
            image_name = Path(file_name).with_suffix(".png").name
        else:
            nombre = slugify(f"{event.room}-{event.start}")
            file_name = ""
            swf_type = ""
            image_name = f"{nombre}.png"
        ...
        image_rel_path = f"record-images/{idx}/{image_name}"
...
def extract_frames(
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        preferred_sources = [
            next((frame for frame in success_frames if frame.name == desired_name), None),
            next((frame for frame in success_frames if frame.stem == Path(preview_filename).stem), None),
        ]
        fallback_source = success_frames[0]
        source_frame = next((frame for frame in preferred_sources if frame is not None), fallback_source)

        target_filename = Path(preview_filename).name
        target_path = dest_dir / target_filename
        shutil.copy2(source_frame, target_path)
